...
app.get('/carsbymaxmileage/:id/:mileage', async (req, res) => {
    try {
        ...
        if (mileage <= 50000) {
            ...
            condition = { $gt: 50000, $lte: mileage };
        } else if (mileage <= 150000) {
            condition = { $gt: 100000, $lte: mileage };
        } else if (mileage <= 200000) {
            condition = { $gt: 150000, $lte: mileage };
        } else {
            condition = { $gt: 200000 };
        }
        ...
        console.error('Error fetching cars by max mileage:', error);
...
app.get('/carsbyprice/:id/:price', async (req, res) => {
    try {
        ...
        if (price <= 20000) {
            ...
            condition = { $gt: 20000, $lte: price };
        } else if (price <= 60000) {
            condition = { $gt: 40000, $lte: price };
        } else if (price <= 80000) {
            condition = { $gt: 60000, $lte: price };
        } else {
            condition = { $gt: 80000 };
        }

        console.log(`Querying with condition: ${JSON.stringify(condition)}`);
